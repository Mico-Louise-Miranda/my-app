...
const TaskCard = ({ id, desc, done, onToggle }) => {
  return (
    ...
    <div className={`bg-gray-900 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow border-l-4 ${done ? 'border-green-500' : 'border-yellow-500'}`}>
      {/* Task details with visual hierarchy using text colors */}
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <p className={`text-lg font-medium ${done ? 'text-gray-400 line-through' : 'text-gray-200'}`}>{desc}</p>
        ...
        <button 
          onClick={() => onToggle(id)} 
          className={`ml-4 p-2 rounded-full ${done ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}
